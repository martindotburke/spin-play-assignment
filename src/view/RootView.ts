...
export class RootView extends BaseViewComponent {
  ...
  protected init(): void {
    this.core.events.on(GameEvent.SHOW_TITLE_SCREEN, () => {
      this.showSplashScreen();
    });
    this.core.events.on(GameEvent.SHOW_GAME_SCREEN, () => {
      this.showGameScreen();
    });

    const background = new Sprite(this.assetManager.getAsset("background.png"));
