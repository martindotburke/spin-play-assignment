...
export class WheelComponent extends BaseViewComponent {
    ...
    constructor(core: CoreUtils) {
        ...
        this.pointer = this.assetManager.createSprite("pointer");

        this.events.addListener(GameEvent.GAME_DATA_RECEIVED, this.init.bind(this));
        this.events.addListener(GameEvent.SPIN_RESULT_RECEIVED, this.onSpinResultReceived.bind(this));
    }
    ...
    private init(): void {
        ...
        this.pointerArc = angle / 10;
        const segmentValues = this.dataStore.gameData?.segmentValues || [];
        if(segmentValues.length != this.settings.segments) {
            throw new Error(`Segment values length ${segmentValues.length} does not match settings segments ${this.settings.segments}`);
        }

        for (let i = 0; i < this.settings.segments; i++) {
            const rotation = i * angle;
            const segment = this.assetManager.createSprite("wheel-slice");
        ...
        const centre = this.assetManager.createSprite("wheel-center");
    ...
    private spinToValue(value: number): void {
        ...
        const { rotationsPerSpin, spinDuration, pullbackDuration, pullbackDistance, landingBounceDuration, landingBounceDistance } = this.settings;
        ...
        console.assert(targetRotation !== undefined, `No target rotation found for value ${value}`);
        ...
        const mainSpinTargetRotation = (currentRotation + offset) + (rotationsPerSpin / 2) * 2 * Math.PI;
        spinTimeline.to(this, {
        ...
        //landing
        const landingSpinTargetRotation = targetRotation + mainSpinTargetRotation + (rotationsPerSpin / 2) * 2 * Math.PI;
        spinTimeline.to(this, {
    ...
    public set rotation (value: number) {
        this.pointer.rotation = (value % this.pointerArc) * -2; // TODO - this should always come to rest at 0
